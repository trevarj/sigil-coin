...
usage() {
  printf 'Usage: deploy-site-remote.sh HOST\n'
}
...
ssh_options=(-o IdentitiesOnly=no)
...
cd "$compose_dir"
container=$(docker compose --env-file .env -f compose.testnet.yml ps -q explorer)
test -n "$container"
test -f "$caddy_source"

rm -rf "$remote_dir/site.next"
install -d -m 0755 "$remote_dir/site.next"
docker cp "$container:/opt/sigilcoin/share/sigilcoin-site/." - |
...
printf 'Site deployment healthy: https://sigilcoin.lol/\n'
